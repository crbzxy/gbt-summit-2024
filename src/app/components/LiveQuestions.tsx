...
const LiveQuestions = ({ tag }: { tag: string }) => {
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      const res = await fetch('/api/questions', {
        ...
        body: JSON.stringify({ userName, userId, question: newQuestion, tag }),
      });
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`absolute bottom-[-54px] right-10 bg-blue-600 text-white p-4 rounded-full shadow-lg cursor-pointer transition-transform transform hover:scale-105 z-50 flex items-center justify-center space-x-2 ${isCooldown ? 'opacity-50 cursor-not-allowed' : ''} ${isButtonVisible ? 'opacity-100' : 'opacity-0'} transition-opacity duration-500`}
      >
        ...
        {userName ? (
          <>
            ...
            <textarea
              ...
              disabled={isLoading}
              className={`w-full p-3 border rounded-lg resize-none focus:outline-none focus:ring-2 mb-4 ${hasError ? 'border-red-500 focus:ring-red-600' : 'border-gray-300 focus:ring-blue-600'
                }`}
              rows={4}
            />
